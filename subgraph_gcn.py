...
def clustergcn(feat_data, labels, adj_matrix, train_nodes, valid_nodes, test_nodes,  args, device, concat=False):
    samp_num_list = np.array([args.cluster_bsize for _ in range(args.n_layers)])
    # use multiprocess sample data
    process_ids = np.arange(args.batch_num)

    cluster_num = int(len(train_nodes)/args.batch_size*args.cluster_bsize)
    print(cluster_num)
    ...
    for epoch in np.arange(args.epoch_num):
        ...
        cur_train_loss, cur_train_loss_all, grad_variance = sgd_step(susage, optimizer, feat_data, labels,
                                                                     ...
                                                                     calculate_grad_vars=bool(args.show_grad_norm) if epoch<200/args.batch_num else False)
        t1 = time.time()
